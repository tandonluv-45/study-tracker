...
import { fetchTasks, fetchGoals, fetchPomodoro, initDatabase } from "@/lib/api";
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const completedForMonth = completions.filter(
        (c) => c.monthKey === monthKey && c.completed
      ).length;
      const mp = currentMonth.goals.length > 0
        ? Math.round((completedForMonth / currentMonth.goals.length) * 100)
        : 0;
      setMonthProgress(mp);

      // Overall + per-month progress
      let totalGoals = 0;
    ...
    load();
  }, [today, monthKey, currentMonth.goals.length]);

  if (!mounted) return null;

  const phaseColors: Record<string, string> = {
    P0: "bg-red-muted text-red",
    P1: "bg-amber-muted text-amber",
    P2: "bg-accent-muted text-accent",
    P3: "bg-blue-muted text-blue",
    "P3→4": "bg-green-muted text-green",
    P4: "bg-green-muted text-green",
  };
    ...
    <div className="animate-fade-in space-y-6">
        ...
        <p className="text-sm text-text-muted">
          ...
          <span
            className={`ml-2 text-xs font-semibold px-2 py-0.5 rounded-md ${
              phaseColors[currentMonth.phase] || "bg-accent-muted text-accent"
            }`}
          >
            {currentMonth.phase}
          </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          sub={todayTasks.total === 0 ? "No tasks yet" : "completed"}
          color="green"
        ...
        <StatCard
          ...
          value={String(pomodoroToday)}
          sub={`${Math.round((pomodoroToday * 25) / 60 * 10) / 10}h focus`}
          color="amber"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {nextMonth && (
          <div className="bg-surface border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-text-muted uppercase tracking-wider">
                Coming Up: {nextMonth.month}
              </h3>
              <span
                className={`text-xs font-semibold px-2 py-0.5 rounded-md ${
                  phaseColors[nextMonth.phase] || "bg-accent-muted text-accent"
                }`}
              >
                {nextMonth.phase}
              </span>
            ...
            <div className="space-y-2">
              ...
              {nextMonth.goals.length > 5 && (
                <p className="text-xs text-text-dim ml-4">
                  +{nextMonth.goals.length - 5} more goals
                </p>
              )}
            </div>
            <div className="pt-3 mt-3 border-t border-border">
              <p className="text-xs text-text-muted">
                {nextMonth.hrsPerWeek} hrs/week &middot; {nextMonth.monthEndGoal}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {roadmap.map((m) => {
            ...
            return (
              ...
              >
                <span className={`text-xs font-semibold ${isCurrent ? "text-accent" : "text-text-muted"}`}>
                  {m.shortMonth}
                </span>
...
function StatCard({ icon, label, value, sub, color, onClick }: {
  ...
  return (
    <button onClick={onClick} className="bg-surface border border-border rounded-xl p-4 text-left hover:border-border-light transition-all group">
